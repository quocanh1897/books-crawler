...
Both sources share the same output pipeline: compress (zstd + global dict)
→ write BLIB v2 bundles → upsert SQLite metadata.

Modes:
    Ingest (default)    Fetch chapters from the source, compress, and
...
Plan file:
    MTC default: data/fresh_books_download.json
    TTV default: data/ttv_books_download.json
...
TTV_DEFAULT_PLAN = SCRIPT_DIR / "data" / "ttv_books_download.json"
