...
function runImport(fullMode: boolean): ImportReport {
  ...
  for (const bookIdStr of entries) {
    ...
    if (booksLoopIdx % progressInterval === 0) {
      ...
      const remaining =
        booksPerSec > 0 ? (entries.length - booksLoopIdx) / booksPerSec : 0;
      logDetail(
    ...
    try {
      ...
      if (bundleMaxIdx !== null && chapterFiles.length > 0) {
        ...
        if (newTxtIndices.length > 0) {
          ...
          if (minNew !== bundleMaxIdx + 1) {
            report.gapErrors++;
            report.gapErrorDetails.push({
              bookId,
              maxBundle: bundleMaxIdx,
              minNew,
            });
      ...
      const actualChaptersSaved = bundleWriter.size;
      if (chaptersThisBook > 0) {
        bundleWriter.flush();
      }

      // Correct chapters_saved with actual bundle count (the initial
      // INSERT used meta.chapter_count which is the API's total, not
      // the number of chapters we actually have on disk).
      if (actualChaptersSaved !== chaptersSaved) {
        sqlite
          .prepare("UPDATE books SET chapters_saved = ? WHERE id = ?")
          .run(actualChaptersSaved, bookId);
      }
