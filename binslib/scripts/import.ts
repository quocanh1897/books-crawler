 ...
 *   npx tsx scripts/import.ts --cleanup          # remove .txt files after import
 */
...
function printReport(report: ImportReport) {
    ...
    const lines = [
        ...
        ...(report.cleanupFilesDeleted > 0
            ? [
                  sep,
                  row(
                      "Cleanup: files:",
                      formatNum(report.cleanupFilesDeleted),
                      c.yellow
                  ),
                  row(
                      "Cleanup: freed:",
...
function appendLog(report: ImportReport) {
    ...
    const entry = [
        ...
        ...(report.cleanupFilesDeleted > 0
            ? [`  cleanup: ${report.cleanupFilesDeleted} files deleted, ${(report.cleanupBytesFreed / (1024 * 1024)).toFixed(1)} MB freed`]
            : []),
...
function runImport(fullMode: boolean): ImportReport {
  ...
  `);
    ...
    let nextGenreId = ((maxGenreId.get() as { max_id: number })?.max_id ?? 100) + 1;

    function resolveGenreId(g: { id?: number; name: string; slug?: string }): number {
    ...
    for (const bookIdStr of entries) {
        ...
        const bookDir = entrySourceMap.get(bookIdStr) || path.join(CRAWLER_OUTPUT, bookIdStr);
