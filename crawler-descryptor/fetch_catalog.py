...
then cross-references with what's already downloaded to identify gaps.
...
def verify_author(client: httpx.Client, author_id: int):
    ...
    for it in items:
        print(f"  {it.get('id'):>7} ch={it.get('latest_index',0):>5} {it.get('name','?')[:50]}")
    return items


def audit(catalog: list[dict]):
    """Cross-reference catalog with local downloads."""
    existing = {}
    for d in os.listdir(OUTPUT_DIR):
        try:
            ...
            continue
        book_dir = os.path.join(OUTPUT_DIR, d)
        ch_count = sum(1 for f in os.listdir(book_dir)
                       if f.endswith(".txt") and f[0].isdigit())
        existing[bid] = ch_count

    downloadable = [b for b in catalog
                    if b.get("first_chapter") and b.get("chapter_count", 0) > 0]
    not_downloadable = [b for b in catalog
    ...
    total_gap = (sum(b["gap"] for b in have_partial) +
                 sum(b["chapter_count"] for b in need_download))
...
def main():
    ...
    parser.add_argument("--verify", type=int, help="Verify author ID has expected books")
    parser.add_argument("--skip-fetch", action="store_true", help="Use existing catalog file")
    ...
    if result["partial"]:
        ...
        for b in result["partial"][:20]:
            print(f"  {b['id']:>7} {b['local']:>5}/{b['chapter_count']:<5} gap={b['gap']:>5} {b['name'][:42]}")
